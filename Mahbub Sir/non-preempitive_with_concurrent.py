...
def priority_scheduler(processes):
    ...
    while processes:
        ...
        highest_priority_process = max(available_processes, key=lambda p: p.priority)
        processes.remove(highest_priority_process)
        completion_time = highest_priority_process.run(current_time)
        completed_processes.append((highest_priority_process, completion_time))
...
if __name__ == "__main__":
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        completed_processes = priority_scheduler(processes)
